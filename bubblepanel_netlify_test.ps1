<#
.SYNOPSIS
  Netlify-safe tester for BubblePanel backend (via /api proxy).
  - DRY RUN default to avoid 504s if backend is synchronous
  - If backend is async, polls /run/{id} until done
#>
...
try {
  ...
  if(-not $DryRun){
    if($result.overays -and $result.overlays.Count -gt 0){ Download-Artifact -ServerPath $result.overlays[0] }
