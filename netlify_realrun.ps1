<# Full OCR through Netlify with async /run polling. Exits if backend is synchronous. #>
...
Write-Host "POST $Base/run (real)" -ForegroundColor Cyan
# MUST return fast with { id } if async is deployed; otherwise Netlify will 504
...
if(-not ($hasId -and $resp.id)){
  Write-Host "SYNC backend detected (no 'id' in /run response). Netlify will 504 on long runs. Deploy async /run first." -ForegroundColor Red
  exit 2
}
...
# Download first artifacts (optional)
