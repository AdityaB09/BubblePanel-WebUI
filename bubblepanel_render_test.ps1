<#
.SYNOPSIS
  Render-only tester for BubblePanel backend (no Netlify proxy).
  - Uploads an image (curl.exe)
  - Calls /run (synchronous or async OK)
  - Polls /run/{id} if async
  - Downloads first overlay/text/jsonl when run completes
#>

param(
  [string]$Base         = "https://bubblepanel-webui-1.onrender.com",
